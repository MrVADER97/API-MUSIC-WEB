...
(function () {
  ...
  function loadITunes(g) {
    ...
    const fetcher = g.mbTag
      ? window.SonidoAPI.tracksByMbTagWithPreviews(g.mbTag, 13, 30, 4)
          .then((tracks) => {
            ...
            return (g.genreNames && g.genreNames.length)
              ? window.SonidoAPI.tracksByTermFilteredByGenre(g.searchTerm, g.genreNames, 13)
              : window.SonidoAPI.tracksByGenre(g.searchTerm, 13);
          })
      : window.SonidoAPI.tracksByGenre(g.searchTerm, 13);
    fetcher
      .then((tracks) => {
        ...
        wrap.innerHTML = tracks
          .map((t) => `<div class="col-12 col-sm-6 col-md-4 col-xl-3">${window.SonidoRender.trackCard(t)}</div>`)
          .join("");
        // Inicializar reproductor para botones
        initGenrePlayer(wrap);
      })
      .catch((err) => {
        console.error(err);
